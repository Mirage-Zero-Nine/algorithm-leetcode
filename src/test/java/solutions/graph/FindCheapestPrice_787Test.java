...
public class FindCheapestPrice_787Test {
    ...
    public void testSourceEqualsDestination() {
        int[][] flight = new int[][]{{0, 1, 10}, {1, 2, 20}};
        assertBoth(3, flight, 1, 1, 0, 0);
    }

    @Test
    public void testNoFlightsAvailable() {
        int[][] flight = new int[][]{};
        assertBoth(4, flight, 0, 3, 2, -1);
    }

    @Test
    public void testDirectCheaperThanMultiStop() {
        int[][] flight = new int[][]{{0, 1, 100}, {1, 2, 100}, {0, 2, 150}};
        assertBoth(3, flight, 0, 2, 1, 150);
    }

    @Test
    public void testMultiStopCheaperThanDirect() {
        int[][] flight = new int[][]{{0, 1, 5}, {1, 2, 5}, {0, 2, 50}};
        assertBoth(3, flight, 0, 2, 1, 10);
    }

    @Test
    public void testStopsConstraintBlocksCheapestPath() {
        int[][] flight = new int[][]{{0, 1, 5}, {1, 2, 5}, {2, 3, 5}, {0, 3, 25}};
        assertBoth(4, flight, 0, 3, 1, 25);
    }

    @Test
    public void testCycleGraphDoesNotLoop() {
        int[][] flight = new int[][]{{0, 1, 3}, {1, 0, 2}, {1, 2, 4}, {0, 2, 20}};
        assertBoth(3, flight, 0, 2, 10, 7);
    }

    @Test
    public void testUnreachableWithinGivenStops() {
        int[][] flight = new int[][]{{0, 1, 2}, {1, 2, 2}, {2, 3, 2}};
        assertBoth(4, flight, 0, 3, 1, -1);
    }

    @Test
    public void testGiantChainCase() {
        int n = 120;
        int[][] flight = new int[n - 1][3];
        for (int i = 0; i < n - 1; i++) {
            flight[i][0] = i;
            flight[i][1] = i + 1;
            flight[i][2] = 1;
